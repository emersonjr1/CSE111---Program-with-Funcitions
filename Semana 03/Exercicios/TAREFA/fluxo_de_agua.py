# ADICIONADO UMA FUNCAO QUE TRANSFORMA KPA EM MCA
...
def main():
    ...
    converter_kpa_para_mca(pressao)
...
def calc_pressao_pela_altura(altura):
    densidade = 998.2
    aceleracao = 9.80665 #m
     
    return (densidade * aceleracao * altura) / 1000 
...
def calc_num_reynolds(diametro_hidraulico, velocidade_fluido):
    densidade = 998.2
    viscosidade = 0.0010016


    return (densidade * diametro_hidraulico * velocidade_fluido) / viscosidade
...
def converter_kpa_para_mca(pressao_kpa):
  
    fator_conversao = 1 / 9.80665
    
    pressao_mca = pressao_kpa * fator_conversao
    print(f"A pressão em MCA é: {pressao_mca}")
    return pressao_mca
